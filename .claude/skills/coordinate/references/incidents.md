...
with a live end-to-end proof posted to the PR (`gh pr comment` with the UAT run, real exit code,
and concrete assertions or bounded DOM/network/log/database evidence). Screenshots are not
required and should not be generated, captured, attached, or preserved.
