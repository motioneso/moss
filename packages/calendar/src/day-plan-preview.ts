...
export function buildDayPlanPreview(input: BuildDayPlanPreviewInput): PreviewDayPlanResponse {
  ...
  // Calendar-busy overlap. A stale cache still reflects real commitments, so both "available"
  // and "stale" feed conflict detection — only "unavailable" (no usable read at all) skips it.
