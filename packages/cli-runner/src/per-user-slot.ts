...
import { purgeOwnedPath } from "./owned-fs.js";
 ...
 * sweep loses nothing. The call's transcript is removed here by path instead, from the shared home
 * and, for a folder a user's slot owns, from that user's home as that user.
 */
export async function clearOwnedStructuredFolders(deps: {
  ...
  for (const name of names) {
    ...
    const identity = { uid: stat.uid, gid: stat.gid };
    await rm(transcriptGlobDir("anthropic", path, homeBase), {
      recursive: true,
      force: true
    }).catch(() => undefined);

    // A call keyed by its user wrote its transcript in that user's home, as that user.
    const user = homeBase === undefined ? undefined : uidSlotUser(homeBase, stat.uid);
    if (homeBase !== undefined && user !== undefined) {
      const agentHome = join(homeBase, "agents", user);
      await (purge ?? purgeOwnedPath)(
        transcriptGlobDir("anthropic", path, agentHome),
        identity
      ).catch(() => undefined);
    }
    await removeOwnedFolder(path, identity, purge).catch(() => undefined);
  }
}
