...
function lintToolInputSchemaPatterns(
  ...
  if (node.properties && typeof node.properties === "object" && !Array.isArray(node.properties)) {
    for (const [key, value] of Object.entries(node.properties as Record<string, unknown>)) {
      lintToolInputSchemaPatterns(value, `${path}.properties.${key}`, errors, depth + 1);
    }
  }
  if (node.items !== undefined) {
    lintToolInputSchemaPatterns(node.items, `${path}.items`, errors, depth + 1);
  }
}
      ...
      for (const entry of obj.assistantTools) {
        ...
        if (tool.inputSchema !== undefined) {
          lintToolInputSchemaPatterns(
            tool.inputSchema,
            `${isNonEmptyString(tool.name) ? tool.name : "?"}.inputSchema`,
            errors,
            0
          );
        }
