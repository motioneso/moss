...
(async () => {
  ...
  try {
      ...
      for (const placement of ['automatic','proposed']) {
        ...
        for (let phase = 0; phase < 4; phase++) {
          ...
          const dimensions = await page.locator('#evening-planning').evaluate(el => ({
            ...
            viewport:innerWidth, scroll:el.querySelector('.planning-scroll').scrollWidth,
            scrollClient:el.querySelector('.planning-scroll').clientWidth,
          }));
          assert(dimensions.width <= dimensions.client + 1 && dimensions.right <= width + 1,
            `Dialog overflows at ${width}, phase ${phase}: ${JSON.stringify(dimensions)}`);
          assert(dimensions.scroll <= dimensions.scrollClient + 1, `Content overflows at ${width}/${phase}`);
          ...
          if ((width === 375 || width === 1440) && placement === 'automatic') {
            await page.locator('#evening-planning').screenshot({ path:path.join(out, `${width}-${phase}.png`) });
          }
        }
      }
    }
    ...
    for (const placement of ['automatic','proposed']) {
      ...
      assert.equal(await page.locator('[data-bike="tomorrow"]').getAttribute('aria-pressed'), 'true');
      ...
      assert.match(await page.locator('.planning-side .draft-entry').innerText(), /review follow-up/);
      ...
      assert.match(await page.locator('.planning-side .draft-entry').innerText(),
      ...
      assert.equal(await page.locator('[data-action="prepare"]').evaluate(el => el === document.activeElement), true);
    }
    ...
    await page.locator('#evening-planning').screenshot({ path:path.join(out, 'changed-plan-review.png') });
    await page.getByRole('button', { name:'Save proposed plan', exact:true }).click();
    assert.match(await page.locator('.planning-side .draft-entry').innerText(), /3:00pm/);
    assert.match(await page.locator('.planning-side .existing-plan').textContent(), /1:00pm/);
    await page.locator('#evening-planning').screenshot({ path:path.join(out, 'changed-plan-saved.png') });
    await page.getByRole('button', { name:'Adjust this plan', exact:true }).click();
    await page.locator('[data-phase="3"]').click();
    await page.locator('[data-include="timeline"]').uncheck();
    ...
    await page.getByRole('button', { name:'Save proposed plan', exact:true }).click();
    await page.getByRole('button', { name:'Preview the morning handoff ↗', exact:true }).click();
    assert.match(await page.locator('.planning-conversation').innerText(), /Existing calendar task blocks remain/);
    assert.doesNotMatch(await page.locator('.planning-side').innerText(), /afternoon stays open/);
    await page.setViewportSize({ width:375, height:1000 });
    await page.locator('#evening-planning').screenshot({ path:path.join(out, 'changed-plan-handoff-phone.png') });
    assert.deepEqual(errors, []);
    fs.writeFileSync(path.join(out, 'checks.json'), JSON.stringify({views, pageErrors:errors,
      checks:['draft resume','prior decisions carried in','reflection correction','lighter day and focus',
        'partial selection','calendar conflict','save failure and retry','both placement settings',
        'morning handoff','unavailable briefing','zero selected blocks','literal notes','Escape/focus return',
        'unsent note retained','current scheduling settings honored on resume',
        'proposed moves and removals preserve existing calendar blocks']}, null,2));
    console.log(`PASS: ${views} responsive conversation views; end-to-end evening planning and recovery scenarios. Crops: ${out}`);
