...
const schedule = [
  {
    ...
    title: "Finish the partnership proposal",
    type: "task",
    meta: "90 min · Work · Due today",
    note: "The outline is finished. Use this block to write the recommendation and send the proposal to Alex.",
    prep: "Proposal outline, pricing comparison"
  },
  { time: "10:00", end: "10:15", title: "A break before the next block", type: "gap" },
  {
    ...
    type: "task",
    meta: "45 min · Work · For your 1pm meeting",
    note: "Review the latest numbers and note the two decisions you need from the group.",
    prep: "Review agenda, project numbers"
  },
  { time: "11:00", end: "12:00", title: "Open time", type: "gap" },
  {
    ...
    title: "Lunch",
    type: "event",
    meta: "1 hour · Personal",
    note: "An existing personal calendar block. Moss leaves it in place."
  },
  {
    time: "13:00",
    end: "14:00",
    ...
    type: "event",
    meta: "1 hour · Video call · Alex + 3",
    note: "Review the revised proposal and decide the scope for the next phase.",
    prep: "Review agenda, project numbers"
  },
  {
    time: "14:00",
    end: "14:30",
    title: "Capture notes & follow up",
    type: "task",
    meta: "30 min · Work · After the review",
    note: "Turn the review decisions into clear next steps while the conversation is fresh."
  },
  { time: "14:30", end: "15:00", title: "Room between commitments", type: "gap" },
  {
    ...
    title: "Team check-in",
    type: "event",
    meta: "30 min · Video call · Your team",
    note: "A short afternoon check-in. Moss plans around this accepted invitation."
  },
  {
    time: "15:40",
    end: "16:00",
    title: "Travel into town",
    type: "event",
    meta: "20 min · Travel buffer · For the repair pickup",
    note: "Time to get to the shop before collection. This buffer keeps the journey visible in the plan."
  },
  {
    time: "16:00",
    end: "16:30",
    ...
    type: "event",
    meta: "30 min · In town · Leave at 3:40",
    note: "Allow 20 minutes to get there. Collection closes at 5pm."
  }
];

function sun() {
  return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" aria-hidden="true"><circle cx="12" cy="12" r="4"/><path d="M12 1v3m0 16v3M1 12h3m16 0h3M4 4l2 2m12 12 2 2M20 4l-2 2M6 18l-2 2"/></svg>';
}
function heading(number, title, meta = "") {
  return `<div class="section-head"><span class="number">${number}</span><h2>${title}</h2><span class="meta">${meta}</span></div>`;
}
function hero() {
  const copy = {
    morning: {
      kicker: "Good morning, Sam / Morning briefing",
      title: "A clear morning.",
      accent: "A full afternoon.",
      summary:
        "The proposal is the important piece today. Start there while the morning is quiet; your review materials are ready for the afternoon. There’s room for a break, lunch, and the trip into town.",
      time: "Prepared at 6:45am",
      temperature: "64°",
      conditions: "Clear until evening",
      forecast: "High 74° · Low 55°"
    },
    day: {
      kicker: "Good afternoon, Sam / Your day, updated",
      title: "The big piece is done.",
      accent: "A little room before what’s next.",
      summary:
        "The proposal is sent and the review is wrapped up. Capture the decisions while they’re fresh, then join your team at 3. Leave at 3:40 for the repair pickup.",
      time: "Updated at 2:10pm",
      temperature: "72°",
      conditions: "Clear through the afternoon",
      forecast: "High 74° · Low 55°"
    },
    evening: {
      kicker: "Good evening, Sam / Evening briefing",
      title: "The proposal is out.",
      accent: "Tomorrow has room.",
      summary:
        "The work you made room for this morning is finished. The review gave the team a direction, and the follow-up is sent. One personal task needs a new place; tomorrow has space for it.",
      time: "Prepared at 7:00pm",
      temperature: "61°",
      conditions: "Clear tonight",
      forecast: "Overnight low 55°"
    }
  }[mode];
...
function agenda() {
  if (mode === "morning" && typeof morningAgenda === "function" && morningSaved)
    return morningAgenda();
  const proposed = scheduling === "proposed" && !accepted;
  const status = proposed
    ? '<p><strong>Your proposed plan.</strong> Three task blocks fit around your commitments.</p><button class="solid" data-action="accept">Accept task blocks</button>'
    : "<p><strong>Your task blocks are on the calendar.</strong> Moss can adjust its blocks when the day changes.</p>";
  return `<section id="day-plan">${heading("01", "Your day, laid out", "Wednesday, September 9")}<div class="plan-note">${mode === "morning" ? status.replace('data-action="accept">Accept task blocks', 'data-action="morning-review">Review task blocks') : status}${mode === "morning" && !proposed ? '<button class="solid" data-action="morning-review">Adjust task blocks</button>' : ""}</div><div class="legend"><span><i></i> ${proposed ? "Proposed task" : "Moss-planned task"}</span><span><i class="fixed"></i> Calendar commitment</span><span><i class="open"></i> Open time</span></div><div class="timeline">${schedule
    .map((s, i) => {
      ...
      const tag =
        s.type === "task" ? (proposed ? "Proposed" : past ? "Completed" : "Flexible") : "";
...
function render() {
  ...
  url.searchParams.set("blocks", scheduling);
  url.searchParams.set("night", quietNight ? "quiet" : "games");
  history.replaceState(null, "", url);
  main.innerHTML =
    hero() +
    `<nav class="section-index" aria-label="Briefing sections"><span class="index-label">IN THIS BRIEFING</span><a href="#day-plan">${mode === "evening" ? "Your day & tomorrow" : "Your day & preparation"}</a><a href="#quick-actions">Quick actions</a><a href="#news">News</a><a href="#sports">Sports</a></nav>` +
    (mode === "day"
      ? '<div class="updated"><strong>2:10pm / One thing changed.</strong>The review finished on time. Your follow-up block is ready, and the rest of the plan still fits.</div>'
      : "") +
    `<div class="briefing-grid">${moduleWidgets()}${mode === "evening" ? evening() : agenda() + preparation()}</div>` +
    news() +
    sports() +
    '<footer class="page-end"><strong>A little more prepared. A little less to carry.</strong><span>Design study · Fictional content · September 9, 2026</span></footer>';
  document.dispatchEvent(new Event("moss:render"));
}

function openDetail(title, body) {
  document.querySelector("#detail-content").innerHTML =
    '<div class="detail-body"><h2 id="detail-title"></h2>' +
    body +
    '<p class="sample-note">Fictional sample content. This preview does not connect to your account.</p></div>';
...
function startConversation() {
  ...
  document.querySelector("#conversation-intro").textContent =
    "Your plan is a starting point. Tell Moss about a new constraint or a different priority.";
  document.querySelector("#moss-question").textContent =
    "I have your schedule and briefing here. What should we adjust?";
  document.querySelector("#replies").replaceChildren();
...
function reply(text) {
  ...
  replies.append(user);
  const moss = document.createElement("p");
  moss.className = "message moss-message";
  moss.textContent =
    "This is where Moss would reconcile your request with the day. This visual study uses sample responses and makes no calendar changes.";
  replies.append(moss);
  document.querySelector("#reply").value = "";
}
...
document.addEventListener("click", (event) => {
  ...
  if (button.dataset.slot) {
    const s =
      mode === "morning"
        ? morningSlot(Number(button.dataset.slot))
        : schedule[Number(button.dataset.slot)];
    openDetail(
      s.title,
      `<dl><dt>When</dt><dd>${s.time}–${s.end}, today</dd><dt>Kind</dt><dd>${s.type === "task" ? "Flexible task block" : "Calendar commitment"}</dd>${s.prep ? `<dt>Ready</dt><dd>${s.prep}</dd>` : ""}</dl><p>${s.note}</p>${s.type === "task" ? '<button class="solid" data-action="adjust">Adjust with Moss ↗</button>' : ""}`
    );
    return;
  }
  if (button.dataset.doc) {
    openDetail(
      button.dataset.doc,
      "<p>Your preparation opens here, with its source and the relevant meeting or task attached.</p><p><strong>For the review:</strong> agree the phase-one scope and confirm who owns the revised timeline. The latest figures and your notes would be available together.</p>"
    );
    return;
  }
  if (button.dataset.story) {
    openDetail(
      ...
      "<p>This is a representative story for reviewing the news and sports layout. The headline, score, and summary are illustrative.</p><p>In the finished briefing, this opens the reporting or game detail, with the source, publication time, and a link to the original coverage.</p>"
    );
    return;
  }
  if (button.dataset.disposition) {
    if (button.dataset.disposition === "date") {
      openDetail(
        "Choose a day",
        '<form id="reschedule"><label for="new-date">Move “Book the bike service” to</label><p><input id="new-date" name="date" type="date" min="2026-09-10" value="2026-09-11" required></p><button class="solid" type="submit">Move task</button></form>'
      );
      return;
    }
    loopDecision =
      button.dataset.disposition === "drop" ? "Removed from your plans." : "Moved to tomorrow.";
    render();
    announce(loopDecision + " Preview only.");
    return;
  }
  switch (button.dataset.action) {
    case "checkin":
      openDetail(
        "How are you feeling?",
        '<form id="checkin-form"><label for="feeling">Choose a feeling</label><select id="feeling" name="feeling" required><option value="">Select a feeling</option><option>Calm</option><option>Hopeful</option><option>Tired</option><option>Overwhelmed</option></select><label for="checkin-note">Anything you want to note? <span class="muted">Optional</span></label><textarea id="checkin-note" name="note" rows="3" maxlength="500" placeholder="A little context, if it helps"></textarea><button class="solid" type="submit">Save check-in</button></form>'
      );
      break;
    case "accept":
      accepted = true;
      render();
      document.querySelector("#day-plan h2").setAttribute("tabindex", "-1");
      document.querySelector("#day-plan h2").focus({ preventScroll: true });
      announce("Three task blocks accepted in this preview.");
      break;
    case "undo-loop":
      loopDecision = "";
      render();
      announce("Task returned to the evening review.");
      break;
    case "ask":
      startConversation("ask");
      break;
    case "adjust":
      detail.close();
      if (mode === "morning") openMorningBriefing("review", "morning-review");
      else startConversation("ask");
      break;
    case "morning-review":
      openMorningBriefing("review", "morning-review");
      break;
    case "prepare":
      openEveningPlan();
      break;
    case "context":
      openDetail(
        "What informed this briefing?",
        "<p>Moss should make its reasoning inspectable without crowding the briefing.</p><dl><dt>Calendar</dt><dd>Accepted meetings, lunch, the repair pickup, and travel time.</dd><dt>Tasks</dt><dd>The proposal deadline, preparation work, and your follow-up.</dd><dt>Last night</dt><dd>Your intention to finish the proposal before the afternoon meetings.</dd><dt>Changed</dt><dd>The review moved from 2pm to 1pm.</dd></dl>"
      );
      break;
    case "briefing":
      if (mode === "morning") {
      ...
      openDetail(
        mode === "evening" ? "Your evening briefing" : "Your morning briefing",
        mode === "evening"
          ? "<p>The proposal you set out to finish is sent. Alex has the recommendation and pricing comparison, and the review ended with agreement on the next phase. Your follow-up is out too.</p><p>The bike service is the only loose task from today. It had no time reserved, and the afternoon filled up. There’s space tomorrow if you still want to do it; nothing depends on deciding tonight.</p><p>Tomorrow’s appointment is at ten. I’d leave the morning around it clear, put the revised timeline after lunch, and preserve some open time. We can shape that together when you’re ready.</p>"
          : "<p>Your main job today is the partnership proposal. You wanted it finished before the afternoon meetings; the plan gives it the first uninterrupted block at 8:30. The outline and pricing comparison are ready.</p><p>The project review moved from two to one. The preparation block at 10:15 still fits, and lunch stays protected. Bring two decisions into the meeting: the first-phase scope and who owns the revised timeline.</p><p>There’s a follow-up block directly after the review, then the team check-in at three. Leave at 3:40 for the repair pickup. The evening is open.</p><p>The plan leaves an open hour before lunch. I’d keep it free for whatever the proposal turns up rather than fill it now.</p>"
      );
      break;
  }
});

document.querySelector("#scheduling").addEventListener("change", (event) => {
...
document.querySelector("#sports-night").addEventListener("change", (event) => {
  quietNight = event.target.value === "quiet";
  render();
});
document.addEventListener("change", (event) => {
  ...
  document.querySelector(".med-count").textContent =
    medicationLog.filter(Boolean).length + " of 2 logged";
  announce(
    (event.target.checked ? "Logged as taken." : "Medication log corrected.") + " Preview only."
  );
});
document.addEventListener("submit", (event) => {
  if (event.target.id !== "checkin-form") return;
  event.preventDefault();
  lastCheckin = document.querySelector("#feeling").value;
  detail.close();
  document.querySelector("#checkin-status").textContent = "Check-in recorded today.";
  const button = document.querySelector("[data-action=checkin]");
  button.textContent = "Check in again";
