...
(async () => {
  ...
  try {
      ...
      for (const blocks of ['automatic','proposed']) {
        ...
        for (const phase of ['read','review']) {
          ...
          const bounds = await page.locator('#morning-briefing').evaluate(el => ({
            width:el.scrollWidth,client:el.clientWidth,right:el.getBoundingClientRect().right,
            scroll:el.querySelector('.planning-scroll').scrollWidth,
            scrollClient:el.querySelector('.planning-scroll').clientWidth,
          }));
          assert(bounds.width <= bounds.client + 1 && bounds.right <= width + 1, `Dialog overflow ${width}/${blocks}/${phase}`);
          assert(bounds.scroll <= bounds.scrollClient + 1, `Content overflow ${width}/${blocks}/${phase}`);
          ...
          views++;
          if (width === 375 || width === 1440) await page.locator('#morning-briefing').screenshot({ path:path.join(out, `${width}-${blocks}-${phase}.png`) });
        }
      }
    }
    ...
    assert.equal(await page.locator('[data-action="briefing"]').evaluate(el => el === document.activeElement), true);
    assert.equal(await page.locator('.slot.proposal').count(), 3, 'Unapplied draft does not change Today');
    ...
    assert.match(await page.locator('.planning-conversation').innerText(), /follow-up stays on your task list/);
    ...
    await page.locator('#morning-briefing').screenshot({ path:path.join(out, 'partial-review.png') });
    await page.locator('[data-morning="save"]').click();
    ...
    assert.equal(await page.locator('.slot.proposal').count(), 1, 'Changing settings does not silently accept saved proposals');
    await page.goto('http://127.0.0.1:8767/?mode=morning&blocks=automatic&brief=open');
    assert.equal(await page.locator('.morning-placement').count(), 0, 'Automatic plan needs no acceptance');
    await page.locator('.morning-nav [data-morning="review"]').click();
    assert.equal(await page.locator('[data-morning="save"]').isEnabled(), false, 'No-op save disabled');
    ...
    assert.equal(await page.locator('.conflict-warning').count(), 0);
    for (const id of [0,2,6]) await page.locator(`[data-morning-status="${id}"]`).selectOption('list');
    await page.locator('[data-morning="save"]').click();
    ...
    assert.equal(await page.locator('#morning-briefing[open]').count(), 1, 'Acceptance keeps the briefing open');
    ...
    await page.keyboard.press('Escape');
    await page.locator('[data-action="briefing"]').click();
    assert.equal(await page.locator('.morning-world').count(), 0, 'Coverage choices survive reopening');
    ...
    assert.equal(await page.locator('#morning-briefing[open]').count(), 0);
    assert.equal(await page.locator('#sports button').first().evaluate(el => el === document.activeElement), true);
    ...
    await page.locator('#morning-scenario').selectOption('save-error');
    await page.getByRole('button', { name:'Accept all time blocks', exact:true }).click();
    assert.equal(await page.locator('main .slot.proposal').count(), 3, 'Failed bulk acceptance changes nothing');
    assert(await page.locator('.save-warning').isVisible());
    await page.getByRole('button', { name:'Retry save', exact:true }).click();
    assert.equal(await page.locator('main .slot.proposal').count(), 0);
    await page.goto('http://127.0.0.1:8767/?mode=morning&blocks=proposed&brief=open');
    ...
    await page.locator('[data-morning-status="6"]').selectOption('list');
    await page.locator('[data-morning-time="0"]').selectOption('09:00');
    assert.equal(await page.getByRole('button', { name:'Accept all time blocks', exact:true }).isEnabled(), false);
    await page.locator('[data-morning-time="2"]').selectOption('10:45');
    await page.getByRole('button', { name:'Accept all time blocks', exact:true }).click();
    assert.equal(await page.locator('main .slot.task').count(), 2, 'Bulk action preserves explicit unscheduling');
    assert.equal(await page.locator('main .slot.proposal').count(), 0);
    assert.deepEqual(errors, []);
    fs.writeFileSync(path.join(out, 'checks.json'), JSON.stringify({ views, pageErrors:errors,
      checks:['reading and sources','preparation materials','partial acceptance','adjustment and conflicts',
        'retained draft','save retry','Today schedule and preparation update','updated task details',
        'no evening plan','unavailable briefing and retry','delayed email','unscheduled tasks keep deadlines','automatic mode needs no approval',
        'one-click accept all and recovery','bulk acceptance respects conflicts and unscheduled tasks',
        'news/sports default on and independent opt-outs','photos and followed-team prose','tonight and quiet night',
        'news/sports links return to Today sections','draft times survive preference changes','saved choices survive preference changes','zero task blocks keeps calendar commitments'] }, null, 2));
    console.log(`PASS: ${views} responsive morning views and interaction/recovery scenarios. Crops: ${out}`);
