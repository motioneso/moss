...
const morningCoverage = {
  news: query.get("news") !== "off",
  sports: query.get("sports") !== "off"
};
...
const morningLabel = (status) =>
  ({ scheduled: "On calendar", proposed: "Proposed", list: "On your task list" })[status];
function initialMorningTasks() {
  return morningTaskIds.map((id) => ({
    id,
...
function morningTasks() {
  return morningSaved || initialMorningTasks();
}
function morningFixed() {
  return schedule.flatMap((slot, id) =>
    slot.type === "event" ? [{ ...slot, id, status: "fixed" }] : []
  );
}
function morningRows(tasks) {
  return [
    ...
    ...tasks
      ...
      .map((task) => ({ ...schedule[task.id], ...task }))
...
function morningConflicts() {
  ...
  return rows.filter((row, i) =>
    rows.some(
      (other, j) =>
        j !== i &&
        minutesOf(row.time) < minutesOf(other.end) &&
        minutesOf(row.end) > minutesOf(other.time)
    )
  );
}
function morningChanges() {
  ...
  return morningDraft.flatMap((task) => {
    ...
    if (task.status === "list")
      return [
        `${old.status === "scheduled" ? "Remove the calendar block" : "Remove the proposed block"} for “${title}”. Keep the task.`
      ];
    if (task.status === "scheduled" && old.status !== "scheduled")
      return [`Schedule “${title}” at ${minuteLabel(minutesOf(task.time))}.`];
    if (task.status === "proposed")
      return [`Keep “${title}” proposed at ${minuteLabel(minutesOf(task.time))}.`];
    return [
      `Move “${title}” from ${minuteLabel(minutesOf(old.time))} to ${minuteLabel(minutesOf(task.time))}.`
    ];
  });
}
...
function morningSnapshot(tasks) {
  ...
  return `<div class="snapshot-heading"><span class="eyebrow">WEDNESDAY, SEPTEMBER 9</span><h3>Your day, in order.</h3></div><div class="morning-schedule">${morningRows(
    tasks
  )
    .map(
      (row) =>
        `<div class="snapshot-entry ${row.type === "task" ? "draft-entry" : "fixed-entry"}"><span>${minuteLabel(minutesOf(row.time))}</span><div>${row.title}<small>${minuteLabel(minutesOf(row.end))} · ${row.type === "task" ? statusText(row) : "Calendar"}</small></div></div>`
    )
    .join("")}</div>${
    tasks.some((task) => task.status === "list")
      ? `<div class="morning-unscheduled"><strong>Without a time block</strong>${tasks
          .filter((task) => task.status === "list")
          .map((task) => `<p>${schedule[task.id].title}</p>`)
          .join("")}</div>`
      : ""
  }`;
}
...
function morningRead() {
  if (morningScenario === "unavailable")
    return '<h3>Your morning briefing isn’t ready.</h3><p>Your calendar and tasks are available. You can review the day while the briefing is prepared.</p><button class="text-button" data-morning="retry-briefing">Try again</button>';
...
function morningReview() {
  ...
  return `<h3>Make the plan fit.</h3><p>Meetings, lunch, and travel stay in place. Adjust the task blocks around them.</p>
    ${morningDraft.some((task) => task.status === "proposed") ? `<button class="solid morning-accept-all" data-morning="accept-all" ${morningConflicts().length ? "disabled" : ""}>Accept all time blocks</button>` : ""}<div class="morning-task-controls">${morningDraft
      .map((task) => {
...
function saveMorning(stayInBriefing = false) {
  ...
  if (stayInBriefing) {
    morningPage = "read";
    drawMorning();
    morningDialog.querySelector(".planning-scroll").scrollTop = scrollTop;
  } else closeMorning();
...
function morningAgenda() {
  ...
  return `<section id="day-plan">${heading("01", "Your day, laid out", "Wednesday, September 9")}<div class="plan-note"><p>${scheduled ? `<strong>${scheduled} task ${scheduled === 1 ? "block is" : "blocks are"} on your calendar.</strong> ` : ""}${pending ? `${pending} ${pending === 1 ? "block awaits" : "blocks await"} acceptance.` : scheduled ? "" : "Tasks remain on your list without time blocks."}</p><button class="solid" data-action="morning-review">${pending ? "Review task blocks" : "Adjust task blocks"}</button></div><div class="legend"><span><i></i> Task block</span><span><i class="fixed"></i> Calendar commitment</span><span><i class="open"></i> Open time</span></div><div class="timeline">${rows
    .map((row, i) => {
      ...
      const gap =
        previous && previous.end < row.time
          ? `<div class="slot gap"><div class="slot-time">${previous.end}<small>${row.time}</small></div><div class="slot-content">Open time</div></div>`
          : "";
      return (
        gap +
        `<div class="slot ${row.type} ${row.status === "proposed" ? "proposal" : ""}"><div class="slot-time">${row.time}<small>${row.end}</small></div><div class="slot-content"><button data-slot="${row.id}"><span class="slot-title">${row.title}</span><span class="slot-meta">${row.type === "task" ? `${minutesOf(row.end) - minutesOf(row.time)} min · ${morningLabel(row.status)}` : row.meta}</span></button></div></div>`
      );
    })
    .join("")}</div>${
    tasks.some((task) => task.status === "list")
      ? `<div class="morning-unscheduled"><strong>Still on your task list</strong>${tasks
          .filter((task) => task.status === "list")
          .map((task) => `<p>${schedule[task.id].title}${task.id === 0 ? " · Due today" : ""}</p>`)
          .join("")}</div>`
      : ""
  }<p class="schedule-end"><strong>The evening is open.</strong> No more commitments after 4:30.</p></section>`;
}
morningDialog.addEventListener("cancel", (event) => {
...
morningDialog.addEventListener("click", (event) => {
  ...
  if (button.dataset.morningJump) {
    ...
    morningDialog
      .querySelector(`#briefing-${button.dataset.morningJump}`)
  ...
  switch (button.dataset.morning) {
    ...
    case "accept-all": {
      ...
      morningDraft.forEach((task) => {
        if (task.status === "proposed") task.status = "scheduled";
      });
      saveMorning(reading);
      break;
    }
    ...
    case "resolve":
      morningDraft.forEach((task) => {
        task.time = schedule[task.id].time;
        task.end = schedule[task.id].end;
      });
      drawMorning(true);
      break;
    case "save":
...
morningDialog.addEventListener("change", (event) => {
  ...
  if (input.dataset.morningCoverage) {
    ...
    drawMorning();
    morningDialog.querySelector(`[data-morning-coverage="${key}"]`).focus();
    return;
  }
  if (input.id === "morning-scenario") {
    morningScenario = input.value;
    morningError = false;
    drawMorning();
    morningDialog.querySelector("#morning-scenario").focus();
    return;
  }
  ...
  if (time !== undefined) {
    ...
    task.end = clockValue(minutesOf(input.value) + duration);
  } else task.status = input.value;
  morningError = false;
  drawMorning();
  morningDialog
    .querySelector(
      `[${time !== undefined ? "data-morning-time" : "data-morning-status"}="${task.id}"]`
    )
...
document.querySelector("#scheduling").addEventListener("change", () => {
  if (morningDraft && !morningSaved) {
    ...
    morningDraft.forEach((task) => {
      if (task.status === before) task.status = after;
    });
  }
...
document.addEventListener("moss:render", () => {
  ...
  summary.textContent =
    proposal.status === "list"
      ? "The proposal is still the priority and is due today. It is on your task list without a time block. Your meetings, lunch, and trip into town are in place."
      : `The proposal comes first, ${proposal.status === "proposed" ? "proposed" : "scheduled"} at ${minuteLabel(minutesOf(proposal.time))}. The review is at one; lunch and the trip into town stay protected.`;
  document.querySelector(".support-section>p").textContent =
    prep.status === "list"
      ? "Preparation is still on your task list without a time block. Bring two decisions into the room."
      : `Preparation ${prep.status === "proposed" ? "is proposed" : "starts"} at ${minuteLabel(minutesOf(prep.time))}. Two decisions to bring into the room.`;
  document.querySelector('[data-doc="Proposal outline"] span').textContent =
    proposal.status === "list"
      ? "For the proposal · Due today"
      : `For your ${minuteLabel(minutesOf(proposal.time))} block`;
  document.querySelector(".support-section:nth-child(3)>p").textContent =
    "Alex moved it from 2pm to 1pm. Lunch stays at noon; the calendar commitments remain in place.";
});
render();
if (query.get("brief") === "open" && !planningDialog.open) {
  mode = "morning";
  render();
  openMorningBriefing();
}
