...
function initialBikeChoice() {
  return loopDecision === 'Moved to tomorrow.' ? 'tomorrow' : loopDecision ? 'existing' : 'undecided';
}
function openEveningPlan() {
  if (!eveningDraft) {
    eveningDraft = {
      correction: false, reflection: '', bike: initialBikeChoice(), bikeDate: '2026-09-11',
      originalBike: loopDecision, pace: 'steady', focus: 'timeline', start: '13:00',
      include: { timeline: true, followup: true, bike: true }, notes: [], composer: '', placement: scheduling,
    };
...
function possibleBlocks(plan) {
  const tasks = [{ id:'timeline', title:'Send the revised timeline', duration:45, why:'From your commitment in the review' }];
  if (plan.correction) tasks.push({ id:'followup', title:'Send the review follow-up', duration:30, why:'You clarified it is still open' });
  if (plan.bike === 'tomorrow') tasks.push({ id:'bike', title:'Book the bike service', duration:15, why:'Carried forward from today' });
...
function plannedBlocks(plan) {
  ...
  return possibleBlocks(plan).filter(task => plan.include[task.id]).map(task => {
...
function existingCalendarBlocks(plan) {
  return !plan ? [] : plan.placement === 'automatic' ? plannedBlocks(plan) : plan.existingBlocks || [];
}
function changesFromSavedPlan() {
  ...
  const changes = before.flatMap(block => {
    ...
    if (!next) return [`Remove the ${minuteLabel(block.start)} block for “${block.title}”. Keep the task.`];
    return next.start !== block.start ? [`Move “${block.title}” from ${minuteLabel(block.start)} to ${minuteLabel(next.start)}.`] : [];
  });
  for (const block of after) if (!before.some(item => item.id === block.id)) changes.push(`Add “${block.title}” at ${minuteLabel(block.start)}.`);
...
function planConflicts() {
  ...
  return plannedBlocks(eveningDraft).some(block => block.start < appointment.end + 30 && block.end > appointment.start - 30);
}
function snapshot(plan, saved = false) {
  ...
  const existing = plan.placement === 'proposed' ? existingCalendarBlocks(saved ? plan : savedEveningPlan) : [];
  const state = saved ? (plan.placement === 'automatic' ? 'Scheduled by Moss' : 'Proposed · not on calendar') : 'Proposed';
...
function reviewPanel() {
  ...
  return `<div class="moss-speaker"><span class="moss-initial">M</span><span>Moss <small>Here’s what will change</small></span></div>
    ...
    <fieldset class="review-actions"><legend>${eveningDraft.placement === 'automatic' ? 'Task blocks to schedule' : 'Task blocks to propose'}</legend>${possibleBlocks(eveningDraft).map(task => `<label><input type="checkbox" data-include="${task.id}" ${eveningDraft.include[task.id] ? 'checked' : ''}><span><strong>${task.title}</strong><small>${task.duration} minutes · ${task.why}</small></span></label>`).join('')}</fieldset>
...
function drawEveningPlan(focusHeading = false) {
  ...
  const focusKey = ['data-reflection','data-bike','data-pace','data-planning-action'].find(key => active?.hasAttribute(key));
  ...
  const body = [reflectionPanel, commitmentsPanel, tomorrowPanel, reviewPanel, finishedPanel, handoffPanel][planningPhase]();
  ...
  if (focusHeading) {
    ...
    planningDialog.querySelector(`[${focusKey}="${CSS.escape(focusValue)}"]`)?.focus({ preventScroll:true });
  }
}
...
function saveEveningPlan() {
  if (eveningDraft.placement !== scheduling) {
    eveningDraft.placement = scheduling; planningPhase = 3; drawEveningPlan(true); return;
  }
  if (planConflicts()) return;
  if (eveningDraft.bike === 'date' && (!/^\d{4}-\d{2}-\d{2}$/.test(eveningDraft.bikeDate) || eveningDraft.bikeDate < '2026-09-10')) {
    planningPhase = 1; drawEveningPlan(true);
    planningDialog.querySelector('#planning-bike-date').reportValidity(); return;
  }
  if (planningScenario === 'save-error') { planningError = true; drawEveningPlan(); return; }
  ...
  tomorrowSaved = true;
  if (eveningDraft.bike === 'tomorrow' && eveningDraft.include.bike) loopDecision = 'Moved to tomorrow.';
  ...
  eveningDraft.originalBike = loopDecision;
  planningError = false; planningPhase = 4;
  render(); drawEveningPlan(true);
}
function refreshEveningSummary() {
  const entry = document.querySelector('[data-action="prepare"]');
  if (!entry) return;
  if (!savedEveningPlan) { if (eveningDraft) entry.textContent = 'Continue preparing tomorrow ↗'; return; }
  const side = document.querySelector('.evening-support .support-section');
  side.innerHTML = snapshot(savedEveningPlan, true) + '<button class="solid" data-action="prepare">Review tomorrow’s plan ↗</button>';
  if (savedEveningPlan.correction) {
    const recap = document.querySelectorAll('.done-item p')[1];
    if (recap) recap.textContent = 'The scope is agreed. You clarified that the follow-up is still open.';
    const summary = document.querySelector('.masthead-summary');
    if (summary) summary.textContent = 'The proposal is sent and the review gave the team a direction. You clarified that the follow-up still needs to go out; it is accounted for in tomorrow’s plan.';
  }
}
function closeEveningPlan() {
  planningDialog.close(); refreshEveningSummary();
  document.querySelector('[data-action="prepare"]')?.focus({ preventScroll:true });
}
planningDialog.addEventListener('cancel', event => { event.preventDefault(); closeEveningPlan(); });
planningDialog.addEventListener('click', event => {
  const button = event.target.closest('button'); if (!button) return;
  if (button.dataset.phase !== undefined) { planningPhase = Number(button.dataset.phase); drawEveningPlan(true); return; }
  if (button.dataset.reflection) {
    eveningDraft.reflection = button.dataset.reflection;
    if (button.dataset.reflection === 'correction') eveningDraft.correction = !eveningDraft.correction;
    if (button.dataset.reflection === 'tired') setPace('light');
    if (!eveningDraft.correction && eveningDraft.focus === 'followup') eveningDraft.focus = 'timeline';
    drawEveningPlan(); return;
  }
  if (button.dataset.bike) { eveningDraft.bike = button.dataset.bike; drawEveningPlan(); return; }
  if (button.dataset.pace) { setPace(button.dataset.pace); drawEveningPlan(); return; }
  switch (button.dataset.planningAction) {
    case 'next': planningPhase = Math.min(planningPhase + 1, 3); drawEveningPlan(true); break;
    case 'close': closeEveningPlan(); break;
    case 'save': saveEveningPlan(); break;
    case 'retry': planningScenario = 'ready'; planningError = false; saveEveningPlan(); break;
    case 'resolve-conflict': eveningDraft.start = '14:30'; drawEveningPlan(); break;
    case 'handoff': planningPhase = 5; drawEveningPlan(true); break;
    case 'edit': eveningDraft = structuredClone(savedEveningPlan); eveningDraft.placement = scheduling; planningPhase = 2; drawEveningPlan(true); break;
  }
});
planningDialog.addEventListener('change', event => {
  const input = event.target;
  if (!['planning-bike-date','planning-focus','planning-start','planning-scenario'].includes(input.id) && !input.dataset.include) return;
  if (input.id === 'planning-bike-date') { eveningDraft.bikeDate = input.value; return; }
  if (input.id === 'planning-focus') { eveningDraft.focus = input.value; setPace(eveningDraft.pace); }
  if (input.id === 'planning-start') eveningDraft.start = input.value;
  if (input.id === 'planning-scenario') { planningScenario = input.value; planningError = false; if (planningPhase >= 4) planningPhase = 3; }
  if (input.dataset.include && Object.hasOwn(eveningDraft.include, input.dataset.include)) eveningDraft.include[input.dataset.include] = input.checked;
  const id = input.id; const include = input.dataset.include;
  drawEveningPlan();
  (id ? planningDialog.querySelector(`#${id}`) : planningDialog.querySelector(`[data-include="${include}"]`))?.focus({ preventScroll:true });
});
planningDialog.addEventListener('submit', event => {
  ...
  if (value) { eveningDraft.notes.push(value); eveningDraft.composer = ''; drawEveningPlan(); planningDialog.querySelector('#planning-note').focus(); }
});
...
if (query.get('plan') === 'open') { mode = 'evening'; render(); openEveningPlan(); }
