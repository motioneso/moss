...
(async () => {
  ...
  try {
    ...
    for (const width of [320, 375, 414, 768, 1280, 1440]) {
      ...
      for (const mode of ['morning', 'day', 'evening']) {
        for (const blocks of ['automatic', 'proposed']) {
          ...
          assert.equal(await page.locator('.score-game').first().evaluate(el => el.classList.contains('followed-game')), true);
          ...
          if ((width === 1440 || width === 375) && blocks === 'automatic') {
            ...
            await page.screenshot({ path: path.join(out, `${mode}-${width}-opening.png`),
              clip: width === 1440 ? { x: 194, y: 64, width: 1246, height: 850 } : { x: 0, y: 0, width, height: 850 } });
            await page.locator('#news').scrollIntoViewIfNeeded();
            await page.locator('#news img').evaluate(img => img.decode());
            await page.screenshot({ path: path.join(out, `${mode}-${width}-news.png`),
              clip: { x: 0, y: 0, width, height: 850 } });
            await page.locator('#sports').evaluate(el => el.scrollIntoView({ block: 'start', behavior: 'instant' }));
            await page.locator('#sports img').evaluate(img => img.decode());
            await page.screenshot({ path: path.join(out, `${mode}-${width}-sports.png`),
              clip: width === 1440 ? { x: 194, y: 0, width: 1246, height: 850 } : { x: 0, y: 0, width, height: 850 } });
          }
        }
      }
    ...
    fs.writeFileSync(path.join(out, 'checks.json'), JSON.stringify({ views, pageErrors: errors,
      checked: ['no horizontal overflow', 'day plan before news and sports', 'placed/proposed blocks',
        'details and Escape', 'carry/reschedule/undo', 'planning conversation', 'literal user text', 'reload resets',
        'multiple scores with followed teams first', 'tonight and quiet-night states', 'loaded news and sports photos',
        'medication logging and correction', 'check-in and state retained across day modes'] }, null, 2));
    console.log(`PASS: ${views} responsive states, 6 quiet-night states, photos, wellness actions, and core preview interactions. Crops in ${out}`);
