...
describe("saved day-plan preview real API boundary", () => {
  ...
  it("live acceptance: resolves done, archived and invisible tasks to their preview eligibility", async () => {
  ...
  it("live acceptance: keeps the saved plan and its preview stable after the server restarts against the same database", async () => {
    const task = await dataContext.withDataContext(userA, (scopedDb) =>
      new TasksRepository().create(scopedDb, {
