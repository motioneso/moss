...
describe("Connectors encrypted foundation", () => {
  ...
  it("loads the built-in Connectors module manifest with the google-sync and imap-sync queues", () => {
    ...
    expect(getBuiltInSqlMigrationDirectories().at(-15)).toContain("packages/chat/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-16)).toContain("packages/ai/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-17)).toContain("packages/email/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-18)).toContain("packages/calendar/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-19)).toContain("packages/notifications/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-20)).toContain("packages/integrations/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-21)).toContain("packages/goals/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-22)).toContain("packages/tasks/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-23)).toContain("packages/connectors/sql");
    expect(getBuiltInSqlMigrationDirectories().at(-24)).toContain("packages/settings/sql");
  });
