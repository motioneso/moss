...
describe("handleExtractFactsJob — memory distillation candidates + no-op degrade", () => {
  ...
  it("dispatches the archive-day job only when archiving is enabled and the thread is not incognito", async () => {
    ...
    const enabledSent: Array<{ readonly queue: string; readonly payload: Record<string, unknown> }> =
      [];
    ...
    const disabledSent: Array<{ readonly queue: string; readonly payload: Record<string, unknown> }> =
      [];
