...
describe("buildDayPlanPreview", () => {
  ...
  it("still reports a calendar_busy conflict for a known commitment even when calendar state is unavailable", () => {
    // "unavailable" means a gap or truncation makes an ABSENCE of conflicts untrustworthy — it
    // does not erase a commitment that was actually returned.
    const plan = planWith([
      {
        id: "block-a",
        kind: "focus",
    ...
    expect(result.conflicts).toEqual([
      {
        ...
        kind: "calendar_busy",
        withBlockId: null,
        detail: 'Overlaps "Team sync" on Google',
        calendarEvent: {
          eventKey: "meeting-1",
          title: "Team sync",
          startsAt: "2026-09-12T16:30:00.000Z",
