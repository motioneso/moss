...
understand approximate calories and nutrients, or recall what preceded a Wellness symptom already
recorded in Moss.

The feature must stay honest about uncertainty. A description, voice transcript, or meal photo cannot
produce measured nutrition or establish that a food caused a symptom. Moss should provide useful
...
already-recorded symptom, and the user has allowed the relevant AI access, Wellness may provide the
symptom time through a declared module boundary. Food can then return meals that preceded it. Moss
...
6. As a user, I want to dictate a meal using Moss's existing voice input, so that I can log hands-free.
7. As a user, I want a voice log to preserve its transcript rather than raw audio, so that it follows
   Moss's existing voice-data behavior.
...
29. As a Wellness user with a recorded symptom, I want to ask what I ate before I felt sick, so that I
    can inspect chronological context.
30. As a Wellness user, I want Moss to use only symptoms already recorded in Wellness, so that it does
    not infer that I was sick from unrelated conversation.
31. As a user, I want a symptom-context answer to state the symptom time and preceding meals without
    claiming a cause, so that Moss does not give unsafe medical conclusions.
32. As a user without Wellness enabled, recorded symptoms, or required consent, I want Moss to explain
...
37. As a user who disables or removes Food, I want Moss to follow the module's stated retain-or-purge
    choice, so that sensitive records do not remain unexpectedly.
...
- Treat the user's timezone as the boundary for calendar dates, daily totals, and natural-language
  date ranges. Persist instants in the project's standard timestamp representation; derive local
  dates at the application boundary.
- Use existing Chat attachment, vision, and voice-transcription capabilities. Food depends on stable
  platform contracts or injected ports, not Chat implementation files. Raw voice audio remains
  transient under the existing voice contract.
...
- Use the existing configured structured-AI/model-resolution path to produce a schema-validated
  nutrition estimate. Do not add a nutrition database, barcode catalog, new provider, or bespoke
  estimation framework for this MVP.
- Text and voice inputs are normalized to bounded meal descriptions. Photo estimation uses the
  existing vision-capable attachment-read boundary. The estimator must return either a valid
  structured estimate, a needs-details result with a bounded clarification question, or a failure;
...
- If estimation uses a durable job, its payload contains only actor id, meal id, estimate revision,
  and idempotency metadata. Workers load private inputs under the actor context. Descriptions,
  nutrition, photo bytes, and transcripts never enter queue payloads.
...
- Creating and retrying a meal is idempotent. Estimate revisions prevent stale asynchronous results
  from overwriting a newer user correction.
...
- For “before I felt sick,” the Wellness module may expose a narrow, consent-aware port returning the
  relevant owner-scoped symptom/check-in timestamps. Food receives only the timestamp and bounded
  symptom label needed for the query, then returns meals in a defined preceding window. The default
  window is 24 hours unless the user asks for another range.
- Symptom-context answers use chronological language such as “you logged these meals before the
  check-in.” Prompts, tool descriptions, and UI must not say a food caused, triggered, diagnosed, or
  treated a symptom.
- AI estimation requires explicit, user-scoped Food AI-processing consent because meal descriptions
  and photos are health-adjacent private data. Wellness AI consent does not silently grant Food
  access. Cross-module symptom context additionally requires Wellness to be enabled and its AI-read
  consent to allow the read.
...
- Logs and metrics contain only bounded identifiers, capture kind, estimate state/revision, duration,
  ...
  values, symptom labels, and AI prompt/response bodies.
- Food registers its records and authorized attachment references with the module export and
  retain/purge lifecycle. Export copy warns that food history is sensitive. Purge deletes Food-owned
  rows and releases Food-owned attachment references through the existing attachment lifecycle.
...
- The primary acceptance test exercises external behavior through both supported entry points: log
  ...
  symptom. Assert public responses and rendered behavior, not prompts or private helper calls.
...
- Cross-module tests prove Wellness shares only an authorized symptom timestamp, Food returns the
  configured preceding window, missing module/data/consent produces an honest explanation, and the
  answer never claims causation.
...
- Data-handling tests prove raw photos/audio, meal text, nutrient values, and symptom labels do not
  appear in logs or job payloads; AI routing and explicit Food consent are honored; purge removes
  Food-owned data and releases attachment references without affecting unrelated Vault files.
- The live acceptance run logs all three capture kinds, corrects one estimate, checks daily totals,
  asks “What did I eat this week?” and “How much protein did I average?”, records a Wellness symptom,
  asks what preceded it, and verifies chronological/non-medical wording at desktop and narrow widths.
...
- Automatic symptom detection from Chat or automatic creation of Wellness records from Food.
