...
3. Live diagnosis (spec requires this AFTER the fix ships): call `/api/mcp` `notes.search` on the
   ...
   the development sign-in details (kept outside the repository (see the memory note named dev-instance-lan-spinup-trusted-origins))) and read the surfaced `cause` in the error text to identify what's actually
