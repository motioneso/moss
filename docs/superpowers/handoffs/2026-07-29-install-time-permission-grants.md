...
the development sign-in details (kept outside the repository (see the memory note named dev-instance-lan-spinup-trusted-origins)) — **dev DB only**. Postgres schema is `app`, not `public`:
