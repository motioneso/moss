...
1. **Run the UAT spec live** on the dev instance (see CLAUDE.md/memory: dev instance
   `http://192.168.50.36:5173`, API `:3000`, login using the development sign-in details (kept outside the repository (see the memory note named dev-instance-lan-spinup-trusted-origins))). Use the `run`
