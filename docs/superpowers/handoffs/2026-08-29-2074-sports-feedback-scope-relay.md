...
- Test account the development sign-in details (kept outside the repository (see the memory note named dev-instance-lan-spinup-trusted-origins)) had no followed teams, so the Followed strip on
