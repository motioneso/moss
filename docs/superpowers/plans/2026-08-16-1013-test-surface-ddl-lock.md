...
export async function dropModuleRolesAtTeardown(
  ...
  options?: { readonly preDropSql?: readonly string[] }
): Promise<void>;
...
- `preDropSql` runs on the **caller's** `client`, **before** the lock is taken. Those statements are
  per-database privilege revokes; running them on the guarded session would target the bootstrap
  database, not the suite's, and would silently revoke nothing.
...
| File | Change |
|---|---|
| `job-search.test.ts`, `job-search-store.test.ts`, `job-search-worker-surface.test.ts`, `job-search-tables-install.test.ts` | inline `DROP ROLE … .catch(() => {})` → `await dropModuleRolesAtTeardown(client, [...], { preDropSql: [...] })`, moving the per-database revokes into `preDropSql` |
...
| `preDropSql` runs on the caller's client, before acquire | spy ordering: every `preDropSql` statement lands on the suite client and precedes the first `pg_advisory_lock` | running them on the guarded session revokes privileges in the wrong database — silent, and exactly the bug that makes teardown look clean while leaking |
...
```bash
pnpm vitest run tests/unit/cluster-ddl-lock-wiring.test.ts > /tmp/1013-p1-unit.log 2>&1; echo "EXIT=$?"
```
...
| Per-database privilege revokes must stay outside the lock | `cluster-ddl-lock.ts:247` (guarded session targets the bootstrap DB) | settled — drove `preDropSql` |
