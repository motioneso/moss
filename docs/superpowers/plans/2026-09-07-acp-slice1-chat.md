...
- **Builds:** in the runner's ACP host, the account slot is allocated by the user's id, the same key the existing per-user runtimes use, never by conversation key (finding 2: one slot per conversation drains the permanent slot pool and gives two conversations of one person two accounts); `HOME` is that slot's own home folder, never the shared base folder; the login reaches the agent by the selected row's mechanism only (finding 1: Claude's stored token in the environment for Claude alone; Codex gets no Claude token and, having no stored login of its own yet, is not ready and refuses to launch: the refusal is the runner's own spawn step, for any row marked not ready, whoever asks; the conversation client's earlier check is only the friendlier message on the product path (ruled 2026-09-08 on Reviewer's finding 2); OpenCode gets neither); the launch-time off-list is applied for every row from `launchOffList(profile)` through the row's own mechanism (finding 3: Claude's disallowed-tools list, Codex's `INITIAL_AGENT_MODE=read-only` in the environment, OpenCode's settings file written into the home with shell and file edits set to deny, moved here from task 6); the Claude-only deny list the host writes today is deleted, so the table is the single source. Two additions from the task 5b review (2026-09-08): with per-user identity off, the launch refuses and never falls back to the shared base folder as `HOME` (the OpenCode deny write would otherwise edit the real settings file under that folder); and the three command-running remote methods (exec start, poll, kill) are unregistered from the runner's connection until the Workshop's own slice keys them by user id, because they still take one slot per session from the same permanent pool and are reachable over the wire with no caller. Three more from Prover's failed checkpoint rerun (2026-09-08, evidence on PR 2427; Ben approved the privileges the same day): (a) **the launcher process itself holds three narrow privileges, never root**: change file ownership, switch account, switch group. "Holds" means the kernel's record for the runner process (`/proc/<pid>/status`) shows exactly those three as effective, and the process runs as the ordinary service account. Prod: `infra/docker-compose.prod.yml` grants the container those privileges (plus the two the container's own boot script needs: one to lock the socket folder it just handed over, and the signal privilege to stop its own children, which run as the ordinary account while the boot script stays root; without it every shutdown signal is refused (Astra-Reviewer, 2026-09-08, source-traced). That grant reaches nobody else: the runner child is started through `setpriv` raising only its three, and the api and worker children are started by Node's account switch, which drops everything. Prod's list is therefore five; the dev compose file lists no privileges on that service and is unchanged), but a grant to the container reaches no child that the boot script starts as another account: Node's account switch at spawn drops every privilege on the way (Astra-Reviewer, 2026-09-08, reproduced: the runner ends with none and the ownership change is refused). So `scripts/start-jarv1s.ts` starts the runner child through `setpriv` from util-linux, present in the image: switch to the host account and group by number, and raise exactly the three as inheritable and ambient, which survive the program start. Only the runner child gets this; the api and worker children are started as today. Dev: the runner runs as a system-level service still running as Ben's account with the same three as ambient privileges (`infra/systemd/jarv1s-dev-cli-runner.service`), and that unit file itself switches per-user separation on on its `ExecStart=` line, running the program through `/usr/bin/env JARVIS_CLI_PER_USER_UID=1 ...`, so the shared env file stays untouched and the install notes never ask for an edit to it (Astra finding 4, twice: the unit as first written read the env file, which says off; an `Environment=` line loses to `EnvironmentFile=` whatever the order, so the value must be set past the point where systemd builds the environment, on the command line itself); the unit's paths are absolute with the account and home folder marked as placeholders, since systemd expands no `~/`; the install commands in the PR notes fill every placeholder, and end with a search for `REPLACE_WITH` in the installed unit that must print nothing. Both compose files switch separation on (`JARVIS_CLI_PER_USER_UID=1`). Both boxes have separation off today and this branch refuses without it, so under the never-break-prod rule this ships in the same PR, not after. (a2) **The three privileges stop at the launcher**: ambient privileges pass to every child across a program start, so an agent started plainly by a privileged runner would hold the power to switch to any account. Every process the runner starts for a person (the ACP agent, and the command path when it returns) is started through `setpriv`: switch to the slot's account and group by number with the supplementary groups cleared (`--clear-groups`, never `--init-groups`: the slots are numbers with no system account entry, so a groups lookup fails the launch outright; Astra, 2026-09-08), drop all inheritable and ambient privileges, then run the program; the working directory is entered after the identity switch, never by Node's `cwd` option before it, since the launcher cannot enter a folder it has handed over. Stopping the agent uses the same route: the launcher cannot signal another account's process with its three privileges, so the stop is sent through the tool as the slot's account to the agent's process group (Astra finding 3); that needs no fourth privilege for the runner (the boot script's signal privilege above is the boot script's, and never reaches the runner). **The stop needs no separate program either** (Astra round 4, 2026-09-08, reproduced: the runtime image has no `kill` program and its build file installs none, so the stop failed and the agent kept running): the signal is sent by Node from the image, the same binary the runner itself runs under, started through `setpriv` as the slot's account, to the agent's process group; no shell in between and no new package in the image. Dev runs the runner from source on a box that does have such a program and would hide the gap, so a unit test asserts the stop command runs nothing but `setpriv` and the runner's own Node binary. A refused stop is an error the close reports, never swallowed, and the session record stays until the process is confirmed gone. The kernel's record for the agent process shows the slot's account and zero in every privilege set; that record is the identity evidence, not the folder's owner. (a3) **Shared parents belong to the launcher, only the leaf is handed over**: the base folder, its `agents` folder and the chat base stay owned by the launcher's own account, mode 0711 (others may pass through, not list), so the launcher can keep creating siblings and each agent can reach its own folder; today the shared `agents` parent is handed to the first person owner-only and the launcher, no longer root, is refused at the next person's folder (Astra finding 3, reproduced with the prod privilege set). The handover with owner-only mode starts at the first level that is one person's: `agents/<user id>` and everything below it, and the conversation key level under the chat base and everything below it. Order matters, because the launcher cannot enter a folder once it is someone else's, and cannot open one that already is: the first launch handed the person's home over owner-only, and the second launch for the same person was refused at its door (Astra round 4, 2026-09-08, reproduced with the three privileges; earlier, Astra finding 2 twice: the conversation folder was handed over before its child was made under it). **So the launcher's own work stops at a person's top level, and everything below it is done as the person.** The launcher creates `agents/<user id>` and the conversation key folder under the chat base only when absent, and hands each over at once, empty, owner-only; when either already exists it checks the kernel's record without opening it (a real folder, not a link, owned by exactly the slot's account and group) and refuses the launch if anyone else owns it. Then a preparation step runs as the slot's account through `setpriv`, the same switch as the agent (numbers, groups cleared, every privilege dropped), running Node from the image: it creates every level below the top (the settings folders, the conversation folder), writes every file the launch puts inside (the OpenCode deny file, merged into an existing one), and exits. Those folders and files are born owned by the person; no handover below a top level happens at all, and no deepest-first ordering is needed any more. The agent launch follows as before, entering its working folder after the switch. Nothing the launcher needs after a person's first launch may live inside that person's tree: the stored login stays in a launcher-owned folder. (b) **A failed ownership handover, or a preparation step that exits with an error, stops the launch there**: the launch fails with that reason (the step's own message), and the cleanup removes only what this launch created (a folder it made, a file it wrote), never a folder or file that already existed (Astra finding 1: the cleanup removed a whole pre-existing home and its contents); no warning-and-carry-on that leaves folders owned by the wrong account. (c) **The checkpoint script connects to the running service**: `scripts/acp-handshake-check.ts` opens the runner's socket with the runner's secret the way the app does, starts Claude for a real Moss account whose user id is given on the command line (never the operator's Unix name standing in for it), completes one exchange, and while the session is alive reads the kernel's record for the agent's process (the runner's session inspection returns the child pid; the script reads `/proc/<pid>/status`) and prints, with the home folder: the account and group, which must equal the slot's (the runner's session inspection returns the slot's account and group with the pid), must not be 0 and must not be the launcher's own; and the permitted, effective, inheritable and ambient privilege sets, which must all be zero (Astra finding 5: a check of two sets passed a root process with live privileges); it builds no runner of its own, and it reads no folder owner as a stand-in for the account (Astra finding 5). The task 4 rerun runs that version and nothing else. A row's "ready" claim is the launch code, not the row text: `providers.ts` says nothing a launch does not do.
