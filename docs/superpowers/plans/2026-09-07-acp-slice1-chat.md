...
- **Builds:** an ACP chat engine behind the session manager for the `chat` profile, chosen by
  ...
  Private conversations run through the new engine too (no old-path fallback): the purge is a runner verb executed as the owning account, the marker is written by the runner at spawn one level above the user-owned working folder, shutdown is stop, then purge, then marker, and the API-side ACP purge and marker code is removed rather than left as a second path (Architect, 2026-09-08; spec section 6, Launch).
- **Files:** `packages/chat/src/live/{engine-selection,runtime,chat-session-manager}.ts`, a new `packages/chat/src/live/acp-chat-engine.ts`, `packages/cli-runner/src/{acp-host,engine-host}.ts` and the runner contract/connection files for the purge verb, `packages/chat/src/manifest.ts`, `tests/uat/specs/2424-acp-chat-lunch.uat.spec.ts` (the live-proof script task 10 runs; modelled on `tests/uat/specs/chat-drawer-private.uat.spec.ts`).
