...
| same | #1207 | routine | Code+DOM live-path proof independently verified clean, CI green. QA RED solely on 2 blocking UAT specs that QA independently confirmed are byte-identical to `origin/main` (pre-existing Moss-rename locator break, tracked as #1481, unrelated to this diff) — manifest reserves zero pre-approved waivers, so **escalated to Ben** (`AWAITING-BEN.md`, `needs-ben` sent); fable-proxy recommends scoped waive-and-merge. Blocking the rest of the fixed merge order until ruled. | `pr1207-relay3`/relay4 (w1:p2K) | `fix-1207-transcript-aria-live` | #1479 |
