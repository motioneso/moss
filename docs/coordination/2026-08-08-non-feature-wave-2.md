...
| same | #1207 | routine | **Ben ruled 2026-08-09: approve fable-proxy's scoped waiver** — code+DOM live-path proof clean, CI green; the 2 failing UAT specs (`1133-chat-attachments`, `runtime-context`) are waived, scoped to this PR only, as a pre-existing Moss-rename locator break (#1481), unrelated to this diff. `gh pr comment` and `gh pr merge` both blocked by the auto-mode classifier this session — ruling recorded here instead; **MERGE-READY, blocked only on classifier — needs a session/method that can execute `gh pr merge`.** | `pr1207-relay3`/relay4 (w1:p2K) | `fix-1207-transcript-aria-live` | #1479 |
