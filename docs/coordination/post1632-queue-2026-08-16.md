...
## Take 38 — Opus adjudication verdict, #1512 gateway.ts scope fork (2026-08-17)

Agent `a7815761c7b504822` ruled. Full verdict is on the agent's own output; summary below is the
operative decision.

- **Root cause confirmed**, but location corrected: `gateway.ts:506-509` (not 536-575), a bare
  `catch {}` in `runHandler()` predating #1512 entirely (commit `eb8ceacbd`, PR #33, 2026-06-08).
- **Reject option (i)** (unwrap `HttpError` messages like `handleRouteError` does). This is not a
  missing unwrap — the swallow is a **deliberate, commented, cross-referenced fail-closed security
  control** ("never leak internals/secrets from a handler throw") because the gateway's sink is the
  **model context / chat transcript**, named separately in CLAUDE.md's "secrets never escape"
  invariant ("...or AI prompts"). #1512's own history is direct evidence against widening this:
  commit `7dc3c8624` exists because a leaked `NotesPathError.message` echoed host paths back
  through a persisted preference. Reopening this control is a closed-design reversal, not a
  coordinator call.
- **Adopt option (ii)** (relax the UAT assertion to the generic swallowed text) as the landing path
  for #1671, **with two binding conditions**: the PR evidence must explicitly name the precision
  gap it leaves open (scenarios (b) and (c') produce byte-identical generic text, so the spec
  proves the guard chain refused but not which specific guard) rather than relax silently, and a
  fast-follow issue must be filed and linked before merge, scoped narrowly as an **opt-in per-tool
  `safeErrors?: true`-style flag** (mirroring the existing `externalContent?: boolean` pattern at
  `module-sdk/src/index.ts:561`, used by `gateway.ts:494-497`) — not a blanket unwrap.
- **Ben's sign-off NOT required** for (ii) + fast-follow — no product code changes, no security
  boundary changes, proof-precision/evidence-honesty call only, squarely coordinator territory.
  Sign-off WOULD be required only if the fleet instead wanted option (i) or any gateway
  error-surfacing change landing inside #1671 itself — not the case here.

**Relayed to relay10** (#1512 lane, pane `w1:pEN`, agent name `notes-1512-relay10`) to apply.
