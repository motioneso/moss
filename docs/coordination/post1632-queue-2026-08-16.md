...
**Date:** 2026-08-16
**Coordinator lock:** label `Coordinator`, **stable anchor = Claude session id `0af0d87c-2a93-4ced-9b55-56dbdfafd9fe`** (match `agent_session.value` in `herdr pane list`; pane `w1:pEZ`, tab `w1:t6`, take 45 — updated each relay; this line, not any historical boot-section mention, is authoritative for Phase 3 step 0 checks). Single-coordinator lock — exactly one pane labelled `Coordinator` whose session id matches this anchor holds authority for the life of the run. ⚠️ **Pane numbers (`w…-N`) reflow on every restart/split/reap — do NOT trust any pane number written in this file as an identifier; resolve the pane fresh by label+session at read time.** Agents escalate to the **label** (routing, re-claimable); the coordinator merges only when its own pane's **session id** (immutable, NOT the pane number) matches this recorded anchor.
