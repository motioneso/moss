...
in-flight reasoning is lost. Re-dispatching a fresh one-shot Opus subagent for the #1013
reconciliation question (not resuming — there is nothing to resume). **Lesson for future relays:**
before closing a predecessor's pane, check whether it has any in-flight `Agent()`-spawned
subagents outstanding (ask it, or check its own manifest note) — those don't survive the pane
closing.
