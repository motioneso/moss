...
- **PR #1671 (1512-notes-path-recheck, security tier)** — pane `w1:pEE`, tab `w1:t19`, session
  `aee74972-c903-4111-9128-b5af12874616`, label "1512 notes path recheck (security) relay7".
  Actively working — mid self-compaction in its own session (81% progress) while holding the
  corrected live-path scope ruling (a + b + c' per the take-36 ruling). Not stalled, no action
  needed. **Do NOT dispatch QA yet** — wait for the live-path proof `gh pr comment`. Once it
  lands: spawn a FRESH Opus QA pass on the whole PR (code + proof, reuse nothing). Green routes to
  Fable-5 sign-off only (security tier, standing delegation `fable-signoff-delegation-waves-3-6`)
  — never merge on a coordinator's own read.
