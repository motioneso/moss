...
**Date:** 2026-08-16
**Coordinator lock:** label `Coordinator`, **stable anchor = Claude session id `e31aa308-ff15-4522-a1cb-ec93566d63cc`** (match `agent_session.value` in `herdr pane list`; pane `w1:pCD`, tab `w1:t6`, take 20 — updated each relay; this line, not any historical boot-section mention, is authoritative for Phase 3 step 0 checks). Single-coordinator lock — exactly one pane labelled `Coordinator` whose session id matches this anchor holds authority for the life of the run. ⚠️ **Pane numbers (`w…-N`) reflow on every restart/split/reap — do NOT trust any pane number written in this file as an identifier; resolve the pane fresh by label+session at read time.** Agents escalate to the **label** (routing, re-claimable); the coordinator merges only when its own pane's **session id** (immutable, NOT the pane number) matches this recorded anchor.
...
| docs/superpowers/specs/2026-08-15-895-required-status-checks.md (exit criteria 2/4/5: gate proof) | #895 | routine | **QA in progress** — all 3 tasks done: throwaway red proof PR #1640 (closed unmerged, CI gate FAILURE as expected) and throwaway green proof PR #1641 (closed unmerged, CI gate SUCCESS w/ skips, deadlock check passed) both recorded on issue #895; real doc-correction PR #1642 open, CI green, criteria 2/4/5 all posted to #895 comments — coordinator spawned QA, awaiting verdict | pr1642-895-docs-correction | w1:pCB | fix/895-auto-merge-notes | #1642 |
