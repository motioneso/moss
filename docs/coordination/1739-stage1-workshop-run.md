...
| #1526 | [1140-D] propagate terminal socket backpressure to the PTY | routine per its own spec (handoff doc had said sensitive — spec wins) | QA found CI red — a test the PR itself added only waits 2 seconds for a connection to close, which flakes under load; the two failures the build agent had flagged as pre-existing/unrelated were confirmed genuinely unrelated. Sent back to the lane to make the test wait for the real close event instead of a fixed timer, then push and re-check CI. 1 of 2 allowed QA cycles used. | pty-1526-relay3 | w1:pHP | 1526-pty-socket-backpressure | 1803 |
