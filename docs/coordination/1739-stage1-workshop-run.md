...
| #1753 | a draft module that runs for its author alone | routine | building (relay2), Task 5 of 10 done. Shape note for #1754: the worker's whole-module gate now simply accepts a draft whose files still match (same as a shipped module), no new per-user parameter — coordinator-approved simplification. Migration numbers renumbered: plan said 0159/0160 but those were already taken by the news package on main; lane is using 0185 (Task 5, done) and will use 0186 for Task 7. | lane-1753-draft-module (relay3) | w1:pJ2 | 1753-draft-module-author-only | - |
...
| #1524 | [1140-B] make whole-league sports follows unique | sensitive (migration; head of a chain — #1572, #906 wait on it) | PR #1807 open, CI running; build agent was about to self-merge on green, corrected — coordinator owns merge after QA; issue must stay open after merge, see today's ruling note | build1524relay2 | w1:pHT | 1524-unique-whole-league-sports-follows | 1807 |
