# Build Handoff — fix-1433-dataset-fetch-warning

**Spec (approved):** docs/superpowers/specs/2026-08-08-non-feature-wave-2.md (your row: #1433)
**GitHub issue:** #1433
**Risk tier:** `routine`
**Worktree:** ~/Jarv1s/.claude/worktrees/fix-1433-dataset-fetch-warning **Branch:** fix-1433-dataset-fetch-warning (off origin/main)
...
- **Explicit log-safety review required against the "secrets never escape" hard invariant** —
  prove no sensitive dataset content or credentials enter logs via the fetch-warning path you're
  changing. Include this proof in your PR description, not just the gate.
- PR open, rebased on `origin/main`.
- This lane touches `packages/datasets/src/client.ts` — not a direct UI surface, so live-path
  proof is not required unless your diff also touches a UI surface. If in doubt, ask.
...
- None. Your seam (`packages/datasets/src/client.ts`) is disjoint from the other three Wave 2
  lanes (#1155 module-registry, #1207 assistant-surface, #1115 task-list-view).
- Merge order for this wave: #1207 → #1155 → #1115 → #1433. You merge last.
