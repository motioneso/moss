# Build Handoff — 1860 module-build environment isolation

**Spec (approved):** docs/superpowers/specs/2026-08-30-1860-module-build-env-isolation.md
**Plan (approved):** docs/superpowers/plans/2026-08-30-1860-module-build-env-isolation.md
**GitHub issue:** #1860
**Risk tier:** `security` — this PR gets adversarial Opus QA + Ben merge sign-off. Build to that bar.
**Worktree:** ~/Jarv1s/.claude/worktrees/build-1860-module-build-env **Branch:** `build-1860-module-build-env` (off origin/main)
...
2. Read the spec and plan above BY SECTION for your current task only — never in full. A full-read
   bloats a fresh context toward the relay threshold before you write any code, which forces a
   premature relay-without-progress. Reading is not progress: BUILD and commit per task.
...
- **Live-path proof posted**: this changes how module builds start up, so it needs the change
  exercised through the real system on a live dev instance, as a `gh pr comment` with the run,
  exit code, and assertions or bounded log evidence. Cannot produce it? Report **code-complete,
  unverified** — never "done". `docs/DEVELOPMENT_STANDARDS.md` → Live-Path Gate.
- Because this is security tier: expect an adversarial Opus review after CI is green, and the PR
  does not merge without Ben's explicit sign-off, posted as a `gh pr comment` verdict first.
...
- No `BYPASSRLS` on runtime app/worker roles; this is exactly the kind of change that invariant
  guards against — if your plan implies loosening a permission boundary to make isolation easier,
  stop and escalate instead.

## Collision notes (from the coordinator)

- This lane runs in parallel with #1784 and #1869 Slice 1 (a one-shot Opus review confirmed all
  three touch completely separate files — no shared file, module, or migration). No coordination
  needed with those lanes.
