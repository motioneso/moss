# Build Handoff — w6c1-secure-context

**Spec (approved):** docs/superpowers/specs/2026-08-09-wave-6-secure-context-and-weather.md
**GitHub issue:** #1402 (lane C, stage 1: timezone floor only)
**Risk tier:** `sensitive` — standard QA plus explicit invariant check plus matched e2e-UAT;
per-merge digest to Ben. Build to that bar.
**Worktree:** ~/Jarv1s/.claude/worktrees/w6c1-secure-context **Branch:** w6c1-secure-context (off origin/main)
...
- **Scope is STAGE 1 ONLY (timezone floor)** — stage 2 (geolocation upgrade) is a separate,
  serialized lane held until Wave 6 lane A (#1403) merges. Do not implement stage 2 in this
  worktree; if the spec's stage boundary is unclear, escalate to the coordinator before building
  past it.
- Spec Exit Criteria (stage 1 portion) met, full gate green **on an isolated gate DB**
...
- Building concurrently with Wave 6 lane A (#1403) and lane B (#900/#1134) against the same spec —
  read the spec's per-lane file split before starting.
- Stage 2 of this same issue (#1402, geolocation) is a separate lane the coordinator will spawn
  only after lane A (#1403) is merged — do not pre-build it here.
